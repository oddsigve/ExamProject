...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        String query2 = " PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> "
        		...
        		+ " { "
        		+ " ?event a dbo:Event ."
        		+ " ?event dbo:startDate ?startDate . Filter(str(startDate) = '1984-09-15' ."
				+ " } ";
        // Argumenter
        if (args != null && args.length == 1) {
            ...
            query2 = new String(
                    Files.readAllBytes(Paths.get(args[0])),package no.uib.infomedia.sinoa.info216;
...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        String query2 = " PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> "
        		...
        		+ " PREFIX : <http://dbpedia.org/resource/> "
        		+ " PREFIX dbo:<http://dbpedia.org/ontology/>"
        		+ " SELECT ?person ?deathDate ?abstract "
        		+ " WHERE "
        		+ " { "
        		+ " ?person a dbo:Person . "
        		+ " ?person dbo:deathDate ?deathDate . Filter(str(?deathDate) = '1984-09-15') . "
        		+ " ?person dbo:abstract ?abstract . Filter(lang(?abstract) = 'en') . "	
				+ " } "
        		+ "LIMIT 100";
        // Argumenter
        ...
        try {
            ...
            q.queryEndpoint(query2, endpointDb);
        }
        catch (Exception ex) {
